...
- [X] T018 [US2] Extend `.github/workflows/ci.yml` with an `evals-full` job: Postgres(pgvector) + Redis service containers + a dev-Vault step (mirroring `smoke`), `load_seed_corpus.py`, then **full `make evals`** so RAG hit@3/MRR + agent tool-selection RUN (no skips); keep the LLM-judge rows report-only (per [contracts/ci-gate.md](contracts/ci-gate.md)) — DONE: added the `evals-full` job to [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml). It mirrors `smoke` (PG+Redis service containers, dev-Vault step) and adds: `checkout` with **`lfs: true`** (so the LFS-stored `seeds/corpus/embeddings.npy` arrives as real bytes, not a pointer), the real `GROQ_API_KEY`/`EMBEDDINGS_API_KEY` Actions secrets forwarded into the Vault-seed step, `make train` (so the classifier-F1 gate has an artifact), `alembic upgrade head`, `load_seed_corpus.py`, then **full `make evals`** — so the offline RAG hit@3/MRR + agent gates now RUN. The LLM-judge rows stay report-only inside `run_evals` (never set the exit code). YAML validated (5 jobs parse). Runs on push-to-`main` + every PR via the existing triggers.
- [X] T019 [US2] Add `GROQ_API_KEY` and `EMBEDDINGS_API_KEY` as GitHub Actions repository secrets and feed them into the `evals-full` job (seed into the job's dev Vault); never commit them — DONE: both repo secrets created on `klc33/sous-chef` via the GitHub Actions secrets REST API (client-side libsodium sealed-box encryption; values pulled from the running local Vault and the owner's stored GH token, so no secret ever passed through a shell arg or got committed). Verified present (names only): `EMBEDDINGS_API_KEY`, `GROQ_API_KEY`. The `evals-full` job consumes them via `${{ secrets.* }}` → forwarded into its dev-Vault seed step.
- [X] T020 [US2] Configure branch protection on `main`: require pull requests (no direct pushes) and mark `ruff`, `mypy`, `gates`, `evals-full`, `smoke` as **required status checks** — DONE: branch protection applied to `main` via the REST API (HTTP 200) — required status checks `[ruff, mypy, gates, evals-full, smoke]`, `strict=true` (branch must be up to date), `enforce_admins=true` (the gate binds the owner too, so the protection is real on this solo repo), and require-a-PR-before-merging (0 required approvals → blocks direct pushes without forcing impossible self-approval). ⚠️ Effect: `git push origin main` is now rejected — all changes (incl. the T018 ci.yml) reach prod only via a green PR merge. Relax `enforce_admins` later if the solo workflow needs it.
...
- [ ] T022 [US2] Verify the gate: open a PR with a deliberately failing red-team probe → required checks red → merge blocked; revert; open a passing PR → merges → Railway deploys that commit (quickstart §C; SC-002) — **READY** once T021 is confirmed. Mechanics: (a) the T018 ci.yml + these tracking updates land via a real PR (now mandatory — `main` is protected), which exercises the full gate incl. `evals-full` (real provider calls) = the *passing-PR* half; merging it triggers the Railway deploy = the deploy half; (b) a throwaway PR adding a deliberately-failing red-team probe proves required-checks-red ⇒ merge-blocked. ⚠️ Opening these PRs costs CI/provider minutes and the merge deploys to **prod** — pending operator go-ahead.
