...
Moved verbatim from the former `app/infra/llm_groq.py` into the `app/infra/llm/` seam (005), now wrapped
in a `GroqClient` satisfying the `LLMClient` Protocol. Groq is chat-only (embeddings live in a separate
provider, see `infra/embeddings.py`). The API key comes from Vault (`GROQ_API_KEY`), never config or code
(golden rule #4). The client is built lazily and cached so the Vault read happens once per process; the
contract/integration tests monkeypatch `_client` (or the `llm` facade's `chat`) and never touch the
...
class GroqClient:
    """`LLMClient` adapter over Groq's OpenAI-compatible chat API (default chat/agent provider)."""
    ...
    def chat(
        ...
        *,
        ...
        `model` defaults to the fast workflow model; the agent passes `settings.groq_agent_model`.
        `tools`, when given, are forwarded so the model may emit tool calls (read on
        `choices[0].message.tool_calls`). `max_tokens` caps the per-call output for the agent's token
        budget. A `429` is retried up to `_MAX_RETRIES` times, waiting the provider's `retry-after` (or
        exponential backoff) between tries; any other error propagates. The full response is returned so
        callers can read the message content, tool calls, and `usage` (for the cumulative token bound).
        """
        settings = get_settings()
